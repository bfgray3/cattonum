...
expected_x1_df_fact <- cattonum_df(data.frame(
  y,
  x1 = c(16, 5, NA, 5, 16, 16),
  x2
))
...
expected_x1_tbl_char <- cattonum_df(tibble(y, x1 = c(16, 5, NA, 5, 16, 16), x2))

expected_x1_tbl_fact <- cattonum_df(tibble(y, x1 = c(16, 5, NA, 5, 16, 16), x2 = factor(x2)))
...
test_that("catto_median() correctly encodes test data.", {
  expect_equivalent( # FIXME: check the attributes

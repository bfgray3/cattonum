...
median_labeler <- function(.grouping, .x, .f) {
...
catto_median <- function(train,
  ...
  median_lkps <- lapply(train[cats],
                        median_labeler,
                        .x = train[[response]],
                        .f = function(...) median(..., na.rm = TRUE))
  
  train[cats] <- encode_from_lkp(train[cats], median_lkps)
  
  if (! test_also) {
    train
  } else {
    test[cats] <- encode_from_lkp(test[cats], median_lkps)

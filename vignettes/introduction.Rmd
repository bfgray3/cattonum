...
* If the functions are asked to encode all factor and character columns, the encoded data will be returned in a `matrix`; otherwise the data will remain in either a `data.frame` or a `tibble`, whichever was passed.
* If `test` is not supplied, the functions return a `matrix`, `data.frame`, or `tibble`, as described above.  Otherwise, they return a length-two list holding the relevant encoded datasets with names `train` and `test`.
